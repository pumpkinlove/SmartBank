...
public abstract class BaseComm {
	...
	private  Vector<Byte> makePackHead(short packType, byte packSeq, byte[] keyData, byte[] ivData, int dataLen){
		  short check_sum = 0;
		  int i = 0;
		  Vector<Byte> v = new Vector<Byte>();
	...
	private int parsePackHead(byte[] packHead, short[] packType, byte[] packSeq, byte[] keyData, byte[] ivData, int[] dataLen){
		short check_sum1 = 0;
		short check_sum2 = 0;
		byte[] tmp = new byte[4];
